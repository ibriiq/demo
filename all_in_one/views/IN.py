...
def get_in_info(request, data, headers, session):
    ...
    with session.post(AccBalance_url, json=data, headers=headers) as response:
            AccBalance = response.json()
    with session.post(GetiCBSsubscriberInfo_url, json=data, headers=headers) as response:
            GetiCBSsubscriberInfo = response.json()
