    ...
    if request.method == "GET":
        ...
        if request.is_ajax():
            ...
            if user is not None:
                ...
                request.session["user"] = {'username': user.username, "pk": user.pk}
    ...
    if request.method == "GET":
        ...
        if request.is_ajax():
            ...
            if int(code) == int(request.session["facode"]):
                user = UserModel.objects.get(username=request.session["user"]["username"])
                login(request, user)
                response["success"] = True
                response["msg"] = "Loggedin successfully"
                ...
                try:
                    ...
                    print('An exception while deleting the user and the facode session keys')
